...
5. build the sensor monitoring sample;
...
8. restore a separate .NET Standard 2.0 consumer from that local NuGet package only;
9. compile that package consumer, including generated `...Async()` and `...Stream()` calls and reverse bridges.

The package-consumer check is deliberately separate from project-reference tests. It verifies what a real NuGet consumer receives rather than only what works inside the repository.
...
- cancellation, timeout, reentrancy, cleanup, disposal, race, and subscriber-exception behavior is covered;
...
- the sensor monitoring sample builds from CI.
